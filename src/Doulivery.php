...
class Doulivery
{
   ...
   *                         Options to configure the Doulivery instance.
   *                         environment - e.g. production or qa (production is default)
   *                         scheme - e.g. http or https
   *                         host - the host e.g. api.doulivery.com. No trailing forward slash.
   *                         port - the http port
   ...
   *
   * @throws DouliveryException Throws exception if any required dependencies are missing
   */
   ...
   *
   * @return void
   * @throws DouliveryException If any required dependencies are missing
   *
   */
  private function check_compatibility()
  {
    if (!extension_loaded('curl')) {
      throw new DouliveryException('The Doulivery library requires the PHP cURL module. Please ensure it is installed');
    }
    if (!extension_loaded('json')) {
      throw new DouliveryException('The Doulivery library requires the PHP JSON module. Please ensure it is installed');
    }
  }
   ...
   *
   * @return void
   * @throws DouliveryException If $channels is too big or any channel is invalid
   *
   */
  private function validate_channels($channels)
  {
    if (count($channels) > 100) {
      throw new DouliveryException('An event can be triggered on a maximum of 100 channels in a single call.');
    }
   ...
   *
   * @return void
   * @throws DouliveryException If $channel is invalid
   *
   */
  private function validate_channel($channel)
  {
    if (!preg_match('/\A[-a-zA-Z0-9_=@,.;]+\z/', $channel)) {
      throw new DouliveryException('Invalid channel name ' . $channel);
    }
  }
   ...
   *
   * @return resource
   * @throws DouliveryException Throws exception if curl wasn't initialized correctly
   *
   */
  private function create_curl($domain, $s_url, $request_method = 'GET', $query_params = array())
  {
    ...
    if ($this->curl === false) {
      throw new DouliveryException('Could not initialise cURL!');
    }
    $curl = $this->curl;
    ...
    curl_setopt($curl, CURLOPT_HTTPHEADER, array(
      ...
      'X-Doulivery-Library: doulivery-php ' . self::$VERSION,
    ));
   ...
   *
   * @return bool|array
   * @throws DouliveryException Throws exception if $channels is an array of size 101 or above or $socket_id is invalid
   *
   */
   ...
   *
   * @return bool|object
   * @throws DouliveryException If $channel is invalid or if curl wasn't initialized correctly
   *
   */
